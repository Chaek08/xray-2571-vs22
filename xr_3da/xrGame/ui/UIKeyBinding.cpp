...
void CUIKeyBinding::InitFromXml(CUIXml& xml_doc, LPCSTR path){
	...
	string_path buf;
	CUIXmlInit::InitListWnd		(xml_doc, strconcat(sizeof(buf),buf,path,":list"),		0, &m_list);
...
//	CUIXmlInit::InitLabel		(xml_doc, strconcat(sizeof(buf),buf,path,":header_3"),	0, &m_header[2]);
	...
	CGameFont* pFake;
	CUIXmlInit::InitFont		(xml_doc, strconcat(sizeof(buf),buf,path,":list:group_name"),0,m_dwGroupColor,pFake);
	CUIXmlInit::InitFont		(xml_doc, strconcat(sizeof(buf),buf,path,":list:item_text"),0,m_dwItemColor,pFake);
