...
void CMotionManager::UpdateAnimCount()
{
	for (ANIM_ITEM_MAP_IT it = get_sd()->m_tAnims.begin(); it != get_sd()->m_tAnims.end(); it++)	{
		...
		for (int i=0; ; ++i) {
			if (0 != smart_cast<CSkeletonAnimated*>(pMonster->Visual())->ID_Cycle_Safe(strconcat(sizeof(s_temp),s_temp,*it->second.target_name,itoa(i,s,10))))  count++;
			else break;
		}
...
{
	string128 s1,s2;
	return smart_cast<CSkeletonAnimated*>(pMonster->Visual())->ID_Cycle_Safe(strconcat(sizeof(s2),s2,*it->second.target_name,itoa(index,s1,10)));
}
