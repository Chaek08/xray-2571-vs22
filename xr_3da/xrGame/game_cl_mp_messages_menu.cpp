	...
	for (u32 i=0; i<10; i++)
	{
		...
		for (u32 s=1; s<=16; s++)
		{
			...
			for (int t=1; t<=GetTeamCount(); t++)
			{
				...
				sprintf(FileName_Voice, "%s%s%d\\voice_%s%d", snd_path, team_prefix, t, SoundName, s);
				sprintf(FileName_Radio, "%s%s%d\\radio_%s%d", snd_path, team_prefix, t, SoundName, s);
				if (FS.exist(fn,"$game_sounds$",FileName_Voice,".ogg") && FS.exist(fn,"$game_sounds$",FileName_Radio,".ogg"))
				{
					...
					pMsgSound->mSound_Voice.create(TRUE, FileName_Voice);	Msg("-- %s Loaded", FileName_Voice);
					pMsgSound->mSound_Radio.create(TRUE, FileName_Radio);	Msg("-- %s Loaded", FileName_Radio);
				}
				else
				{
					R_ASSERT(0);
